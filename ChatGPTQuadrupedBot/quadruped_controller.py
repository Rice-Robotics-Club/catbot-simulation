...
class QuadrupedController(Node):
    def __init__(self):
        ...
        self.joint_publisher = self.create_publisher(Float64MultiArray, '/joint_commands', 10)

        # Gait settings
        self.step_height = 0.05  # Max foot lift during step (meters)
        self.step_length = 0.1  # Step length in x-direction (meters)
    ...
    def calculate_leg_positions(self, time):
        # Phases: LF=0, RF=0.5, LH=0.5, RH=0
        phases = [0.0, 0.5, 0.5, 0.0]  # LF, RF, LH, RH
        positions = []
    ...
    def calculate_joint_angles(self, foot_positions):
        ...
        for x, y, z in foot_positions:
            ...
            l1 = 0.1  # Hip to knee length
            l2 = 0.1  # Knee to foot length
